...
export default function CategoryUpdateModal({
  ...
  const handleUpdate = async (values: any) => {
    try {
      ...
      const responseData = await apiClient.put(`admin/platform-category/${category?.id}`, payload);
      ...
      if (responseData.data.isWarning) {
        await Swal.fire({
          ...
          if (result.isConfirmed) {
            const responseData = await apiClient.put(`admin/platform-category/${category?.id}`, {
              ...payload,
              isConfirm: true,
            });
            if (responseData.data.isSuccess) {
